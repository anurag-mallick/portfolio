    ...
    const animate = () => {
      ...
      connections.forEach(([id1, id2]) => {
        ...
        if (n1 && n2) {
          ...
          const isBlocked =
            n1.status === "disrupted" || n2.status === "disrupted";
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <Card className="col-span-1 p-6 bg-zinc-900/50 border-white/10 backdrop-blur-md h-fit">
        ...
        <div className="space-y-4 mb-8">
          <Button
            onClick={() =>
              setDisruptionType(disruptionType === "STRIKE" ? "NONE" : "STRIKE")
            }
          ...
          <Button
            onClick={() =>
              setDisruptionType(
                disruptionType === "WEATHER" ? "NONE" : "WEATHER",
              )
            }
          ...
          <Button
            onClick={() =>
              setDisruptionType(disruptionType === "FUEL" ? "NONE" : "FUEL")
            }
        ...
        <Card className="p-0 overflow-hidden border-white/10 bg-black/80">
          ...
          <div className="absolute bottom-4 right-4 max-w-sm bg-black/80 backdrop-blur border border-[#00f3ff]/30 rounded-xl p-4 shadow-[0_0_15px_rgba(0,243,255,0.1)]">
            ...
            <p className="text-xs text-white/80 leading-relaxed">
              Simulates my{" "}
              <span className="text-white font-semibold">₹5 Lakh/month</span>{" "}
              Google Maps optimization algorithm at Shiprocket. Reduced delivery
              failures by 18% during monsoon season.
            </p>
          </div>
        </Card>
      </div>
...
function StatBox({ label, value, change, inverse }: any) {
  ...
  const color =
    change === 0 ? "text-white" : isBad ? "text-[#ff0055]" : "text-[#00ff99]";
