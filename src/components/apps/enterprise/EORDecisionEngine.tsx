...
// --- Sub-components (Moved outside to satisfy React pure components rules) ---

const Sankey = ({ calc }: { calc: any }) => {
  const height = 300;
  const width = 800;
  const nodeWidth = 20;
...
const Control = ({ label, value, min, max, step = 1, onChange }: any) => {
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <Card className="lg:col-span-4 p-6 bg-zinc-900/50 border-white/10 flex flex-col gap-6">
        ...
        <div className="space-y-6">
          <div>
            <label className="text-[10px] uppercase font-bold text-muted-foreground block mb-3">
              Target Countries
            </label>
      ...
      <div className="lg:col-span-8 flex flex-col gap-6">
        <Card className="p-6 bg-zinc-900/50 border-white/10">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-white uppercase tracking-tighter">
              Landed Cost Flow (Annualized)
            </h3>
            <div className="text-2xl font-black text-[#00f3ff] font-mono">
              ${(calc.landed / 1000000).toFixed(2)}M
            </div>
          </div>
          <Sankey calc={calc} />
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <ResultCard
            label="REC. STRUCTURE"
            val={
              selectedCountries.length > 3 ? "ENTITY / EOR" : "EOR EXCLUSIVE"
            }
        ...
        {/* Impact Box */}
        <div className="bg-linear-to-br from-zinc-900 to-black border border-[#ff00ff]/30 rounded-xl p-5 shadow-[0_0_20px_rgba(255,0,255,0.1)]">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-2 h-2 rounded-full bg-[#ff00ff] animate-pulse" />
            <span className="text-xs font-bold text-[#ff00ff] uppercase tracking-widest">
              Business Impact
            </span>
          </div>
          <p className="text-sm text-white/80 leading-relaxed">
            Decision logic derived from my work on the{" "}
            <span className="text-white font-semibold underline decoration-[#ff00ff]/50">
              Avance EOR platform
            </span>
            . Powered complex multi-country expansion models for clients,
            contributing to €275k ARR.
          </p>
        </div>
      </div>
            ...
            >
              <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
                <h2 className="text-2xl font-bold flex items-center gap-2">
                  <FileText className="w-6 h-6 text-[#ff00ff]" /> STRATEGIC EOR
                  PROPOSAL
                </h2>
                <button
                ...
                <div className="max-w-2xl mx-auto space-y-8">
                  <div className="flex justify-between items-start border-b border-white/20 pb-8">
                    <div>
                      ...
                      <div className="text-xs tracking-widest uppercase text-muted-foreground font-mono">
                  ...
                  <section>
                    <h4 className="text-white font-bold text-lg mb-4 uppercase tracking-tighter">
                      1. EXECUTIVE SUMMARY
                    </h4>
                    <p>
                      Based on your input of {headcount} employees across{" "}
                      {selectedCountries.join(", ")}, we recommend a{" "}
                      <strong>Hybrid Employment Strategy</strong>. For countries
                      like {selectedCountries[0]}, a direct entity setup is
                      justifiable, whereas{" "}
                      {selectedCountries[1] || "remaining regions"} should be
                      handled via EOR to mitigate high compliance risks.
                    </p>
                  </section>
                  ...
                  <section className="bg-white/5 p-6 rounded-xl border border-white/10 font-mono">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between border-t border-white/10 pt-2 font-bold text-[#ff00ff]">
                        ...
                        <span>${calc.landed.toLocaleString()}</span>
                      </div>
                    </div>
                  </section>
...
function ResultCard({ label, val, sub, icon, color = "text-white" }: any) {
  return (
    <Card className="p-4 bg-zinc-900/40 border-white/5 flex flex-col gap-2">
      ...
      <div className={`text-lg font-black tracking-tighter ${color}`}>
        {val}
      </div>
      {sub && (
